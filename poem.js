...
let currentFrame = updateSentenceFrame(); // Get a random sentence frame

function updateSentenceFrame() {
  const randomIndex = Math.floor(Math.random() * poeticSentenceFrames.length);
  console.log("sentence frame end. new frame:" + randomIndex);
  return poeticSentenceFrames[randomIndex];
}

// Function to log one word at a time
async function logNextWord() {
  if (currentWordPosition >= currentFrame.split(" ").length) {
    currentFrame = await updateSentenceFrame();
  }
  ...
  const replacedFrame = await currentFrame.replace(
    ...
    (match) => {
      ...
      switch (partOfSpeech) {
        ...
        case "adjective":
          ...
          if (currentWordPosition === 0) {
            adjective = adjective.charAt(0).toUpperCase() + adjective.slice(1); // Capitalize the first word
          }
          return adjective;
        case "adverb":
          let adverb =
  ...
  if (currentWordPosition < words.length) {
    ...
    currentFrame = updateSentenceFrame();
...
document.addEventListener("keydown", logNextWord);
